    ...
    match steps with
      ...
      | step :: rest ->
          ...
          if not (is_assert_false step) && not vars_in_use && not let_vars_in_use
            ...
            | Assert (f, _, _) when opt_exists (has_premise f) in_assume ->
                ([], steps, true)  (* proof invoked last assumption as a premise, so exit scope *)
            | Assert (Const ("%⊥", _, range), _, _) ->
...
let is_lambda_or_set_comp f = match f with
  | Lambda _ | App (Const ("{}", _, _), _, _) -> true
  | _ -> false
  ...
  let rec check vars tsubst formula : (tsubst * typ * formula) list =
    match formula with
      ...
      | App (App (Const ("_", _, _), f, _), g, _) ->
          ...
          check vars tsubst h
      | App (Const (q, _, _), Lambda (x, (Sub _ as typ), g), _)
            when q = "(∀)" || q = "(∃)" || q = "{}" ->
      ...
      | App (Const ("{}", _, _), f, _) -> check vars tsubst f
      | App (f, g, range) ->
          let possible = 
...
let check_dup_const env id typ kind range =
  if exists (eq_type typ) (id_types env id) then (
...
let inductive_axioms id constructors : statement list =
  ...
  let var_for typ v = if eq_type typ t then [] else [v] in
  let fs1 =
  ...
  let f3 =
    ...
    let preserves (c, ctyp) =
      if eq_type ctyp t then app p (const c t) else
...
let infer_type_definition env id constructors : statement list =
  ...
  let cs = map (check_const (type_decl :: env)) constructors |> map (fun (c, typ) ->
    if not (eq_type typ (base_type id)) &&
       not (eq_type typ (Fun (base_type id, base_type id))) then (
...
let infer_def_formula env f : id * typ * formula =
  ...
  match f with
    ...
    | App (App (Const ("(→)", _, _), g, _), f, _)->
        let (c, args) = collect_args f in
        let (c, args) = match c with
          | Const (c, _, _) when c.[0] = '@' -> (hd args, tl args)
          | _ -> (c, args) in
...
let mk_thm env lenv contra f by : statement list = Theorem {
    label = ""; name = None; formula = top_infer env f;
    steps = []; by; is_step = true; range = range_of f;
    on_contra_path = contra } :: lenv

let assert_chain env lenv contra thm_num f : statement list list * formula =
  ...
  (map2 (mk_thm env lenv contra) eqs reasons, concl)

let thm_add_by by stmt : statement = match stmt with
  | Theorem thm -> Theorem { thm with by = thm.by @ by }
  | _ -> failwith "set_thm_info"

let new_hyp f : statement = Hypothesis { label = "hyp"; formula = f; name = None }

let block_name (Block (step, _)) : string option = match step with
  | Assert (_, _, name) -> name
  | _ -> None

let rec leads_to_false (Block (step, blocks)) =
  is_assert_false step || not (is_assume step) && exists leads_to_false blocks

let rec blocks_steps env lenv contra thm_num blocks : statement list list * formula =
  match blocks with
    | [] -> ([], _true)
    | block :: rest ->
        let contra1 = contra || exists leads_to_false blocks in
        let (fs, concl) = block_steps env lenv contra1 thm_num block in
        let hyp = Hypothesis { label = "hyp"; formula = top_infer env concl;
                               name = block_name block } in
        let (gs, final_concl) = blocks_steps (hyp :: env) (hyp :: lenv) contra thm_num rest in
        ( fs @ gs,
          if rest = [] then concl
...
and block_steps env lenv contra thm_num (Block (step, children)) : statement list list * formula =
  let child_steps stmts = blocks_steps (stmts @ env) (stmts @ lenv) contra thm_num children in
  ...
  match step with
    | Assert (f, reason, _) ->
        ...
        let steps, concls = unzip (map (assert_chain env lenv contra thm_num) fs) in
    ...
    | Let ids_types ->
        let resolve_subtype (id, typ) = match typ, check_type env typ with
          | Sub f, (Sub f' as t) ->
              ...
              let hyp = new_hyp (app f' (const id ptyp)) in
              [(id, f)], [hyp], (id, ptyp)
          | _ -> [], [], (id, typ) in
    ...
    | LetDef (_id, _typ, g) ->
        ...
        let (fs, concl) =
          child_steps [new_hyp f; mk_const_decl id typ] in
    ...
    | Assume a ->
        ...
        let decls = new_hyp (top_infer (decls @ env) f) :: decls in
    ...
    | IsSome (ids_types, g, reason) ->
        ...
        (mk_thm env lenv contra ex by :: fs,
...
let rec expand_proof id name num env steps proof_steps :
  ...
  let (_, concl) = blocks_steps env [] false "" blocks0 in
...
and find_constructors env of_type : (id * typ) list =
  env |> filter_map (function
    | ConstDecl { id; typ; constructor } ->
        if constructor && eq_type (target_type typ) of_type
...
and combinations arg_map types : id option list list =
  match types with
    | [] -> [[]]
    | t :: types ->
        let cs = match assoc_eq eq_type t arg_map with
          | [] -> [None]
...
and check_equations env id typ fs =
  ...
  let is_constructor (c, typ) =
    mem_id_type (c, typ) (assoc_eq eq_type (target_type typ) arg_map) in
  let check_arg y z =
    if y <> z then match y with
      | App (Const (c, typ, _), y', _) ->
  ...
  let rec check_recursion ys f = match f with
    | App _ ->
        ...
        if is_const_id_type id typ c then iter2 check_arg ys zs;
    ...
    match f with
      | Eq (f, g) ->
          ...
          let check_arg = function
            | Var (x, typ, _) ->
                check "variable not quantified" (mem_id_type (x, typ) xs); None
            | Const (c, typ, _) -> check "not a constructor" (is_constructor (c, typ)); Some c
            | App (Const (c, c_typ, _), Var (x, x_typ, _), _) ->
                check "invalid constructor" (is_constructor (c, c_typ) && mem_id_type (x, x_typ) xs);
                Some c
...
and infer_definition env id_type recursive defs justification : statement list =
  ...
  let justify =
    ...
    Theorem { label = thm_id; name; formula = j; steps; by;
              is_step = false; range = empty_range; on_contra_path = false } in
  let gs =
...
and infer_stmt env stmt : statement list =
  match stmt with
    ...
    | HAxiomGroup (num, id_typ, haxioms) ->
        ...
        let (_, f) = blocks_steps env [] false "" blocks in
    ...
    | HTheoremGroup (num, htheorems) ->
        ...
        let check env htheorem : statement list * statement =
          ...
          let stmt = Theorem { label = id; name; formula = f; steps = stmts;
                               by; is_step = false; range; on_contra_path = false } in
          (stmt :: env, stmt) in
...
let encode_id consts typ id : id =
  if id = _type || mem id logical_ops then id
    ...
    if n > (if mem_id_type (id, typ) consts then 1 else 0) then (
...
let encode_formula consts f : formula =
  let rec encode f =
    match collect_args f with
      | (Const (":", _, _), [g]) ->  (* type ascription *)
          encode g
      | (Const (c, typ, _), args) when is_tuple_constructor c ->
      ...
      | _ ->
        match f with
          | Const (id, typ, _) ->
              Const (encode_id consts typ id, encode_type typ, empty_range)
          | Var (id, typ, _) ->
              Var (id, encode_type typ, empty_range)
          | App (f, g, _) ->
  ...
  let rec check vars f : typ * formula = match f with
    ...
    | App (g, h, _) ->
        ...
        let typ = match g_type, h_type with
          | Fun (Fun (Type, t), u), Pi (_, t') when eq_type t t' ->
              u (* allow * → τ to match Πσ.τ, like in unify_types_or_pi *)
          | Fun (t, u), t' when eq_type t t' -> u
    ...
    | Eq (g, h) ->
        let (g_type, g) = check vars g in
        let (h_type, h) = check vars h in
        if not (eq_type g_type h_type) then (
